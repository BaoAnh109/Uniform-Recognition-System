...
=================================================
Các tối ưu chính:
  - Voting có thể chuyển đổi: ratio-based hoặc vote_threshold cố định
...
UNIFORM_MODEL_PATH = r"best_agu.pt"
...
def compute_vote_threshold_status(history: list[int], vote_threshold: int) -> tuple[str, sv.Color]:
    """
    Cơ chế voting cố định theo số phiếu.
    - Nếu số phiếu đúng >= vote_threshold => PASS
    - Nếu số phiếu sai  >= vote_threshold => FAIL
    - Ngược lại => Verifying...
    """
    total = len(history)
    threshold = max(1, int(vote_threshold))
    if total < threshold:
        return "Verifying...", COLOR_WAIT

    pass_votes = sum(history)
    fail_votes = total - pass_votes

    if pass_votes >= threshold:
        return "ĐÚNG ĐỒNG PHỤC", COLOR_PASS
    if fail_votes >= threshold:
        return "SAI ĐỒNG PHỤC", COLOR_FAIL
    return "Verifying...", COLOR_WAIT


def compute_voting_status(cfg, history: list[int], single_image: bool) -> tuple[str, sv.Color]:
    if single_image:
        return ("ĐÚNG ĐỒNG PHỤC", COLOR_PASS) if history and history[-1] == 1 else ("SAI ĐỒNG PHỤC", COLOR_FAIL)

    mode = str(getattr(cfg, "voting_mode", "ratio")).strip().lower()
    if mode == "threshold":
        return compute_vote_threshold_status(history, getattr(cfg, "vote_threshold", 3))

    return compute_ratio_status(
        history=history,
        pass_ratio=getattr(cfg, "pass_ratio", 0.65),
        fail_ratio=getattr(cfg, "fail_ratio", 0.35),
        min_votes=getattr(cfg, "min_votes", 5),
    )
...
class AppState:
    def __init__(self):
        ...
        # voting
        self.voting_mode   = "ratio"   # ratio | threshold
                ...
                if detections is not None and len(detections.xyxy) > 0:
                    ...
                    if should_check and crops_list:
                        ...
                        for res, tid in zip(uni_results, tids_list):
                            ...
                            text, color = compute_voting_status(st, history, single_image=single_image)
                            ...
                            if len(history) == 1 or len(history) == st.min_votes:
                                ...
                                self._log(
                                    ...
                                    f"area={info['best_area_ratio']:.3f} score={info['total_score']:.3f} ratio={ratio:.2f} mode={st.voting_mode}"
                                )
...
class App(ctk.CTk):
    ...
    def _build_header(self):
        ...
        ctk.CTkLabel(
            hdr,
            text="Voting ratio/threshold | Uniform score/class/conf | Toggle segmentation",
            font=("Arial", 12)
    ...
    def _build_control_panel(self):
        ...
        self._section(ctrl, "🧮  Voting")
        self._option_menu(
            ctrl, "Voting Mode", ["ratio", "threshold"], self.cfg.voting_mode,
            lambda v: setattr(self.cfg, "voting_mode", str(v))
        )
        self._slider(
        ...
        self._slider(
            ctrl, "Vote Threshold", 1, 20, self.cfg.vote_threshold,
            lambda v: setattr(self.cfg, "vote_threshold", int(v)), integer=True
        )
        ctk.CTkLabel(
            ctrl,
            text="ratio: dùng tỉ lệ | threshold: dùng số phiếu cố định",
            font=("Arial", 10), text_color="gray", anchor="w"
        ).pack(fill="x")
    ...
    def _option_menu(self, parent, label, values, default, callback):
        row = ctk.CTkFrame(parent, fg_color="transparent")
        row.pack(fill="x", pady=3)
        ctk.CTkLabel(row, text=label, anchor="w").pack(fill="x")
